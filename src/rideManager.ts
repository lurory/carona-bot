...
export default class RideManager {
  ...
  public removeRide(chatId: number, rideInfo: { userId: number; direction: string }): boolean {
    return db.updateGroup(
      chatId,
      {
        $unset: {
  ...
  public async listRidesAsString(chatId: number): Promise<string> {
    const totalRides = (await db.scrapeGroupRides(chatId)) as Ride[]
    let message: string

    // No rides
    if (totalRides.length == 0) return ''
